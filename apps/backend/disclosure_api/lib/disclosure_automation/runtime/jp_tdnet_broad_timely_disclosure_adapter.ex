defmodule DisclosureAutomation.Runtime.JPTDnetBroadTimelyDisclosureAdapter do
  @moduledoc false
    ...
    {:ok,
     [
       %{
         ...
         parser_key: source.parser_key || "jp_tdnet_broad_timely_disclosure_v1",
         ...
         payload: %{
           ...
           "document_date" => item.row_date,
           "fact_summary_ko" => summarize_ko(item, pdf_text),
           "why_important_ko" =>
             "TDnet broad timely disclosure fixture preserving official row metadata, PDF token identity, raw TDnet code, and normalized security code.",
  ...
  def normalize(%SourceRegistry{} = source, raw_event, opts \\ []) do
    ...
    event_id = "jp.tdnet.#{security_code}.#{row_date_compact}.#{@canonical_event_type}.#{event_family}.#{pdf_document_token}"
    ...
    contract_v1 = %{
      ...
      "issuer_ids" => %{"security_code" => security_code, "tdnet_raw_row_code" => payload["tdnet_raw_row_code"]},
      "issuer_name_local" => payload["issuer_name"],
      "issuer_name_en" => payload["issuer_name"],
      "headline_local" => payload["title"],
      "headline_ko" => "TDnet broad timely disclosure update",
      ...
      "risk_flags" => ["jp_tdnet_broad_fixture_v0", "pdf_token_identity_used", "category_unknown_not_inferred"],
      "portable_citations" => [
        %{"source_name" => "TDnet current-list row", "claim_supported" => "disclosure date/time, code, company display name, title, exchange, and PDF token", "note" => payload["row_list_url"]},
        %{"source_name" => "TDnet PDF attachment", "claim_supported" => "company name, security code, exchange, title, and disclosure body used for v0 normalization", "note" => payload["attachment_url"]}
      ]
    }
  ...
  defp discovery_item_from(row) do
    ...
    stable_external_id = "TDNET:#{normalized_security_code}:#{String.replace(row_date, "-", "")}:#{disclosure_time_compact}:#{pdf_document_token}"
  ...
  defp target_row?(item, %SourceRegistry{} = source) do
    ...
    allowed = filter["pdf_document_tokens"] || filter[:pdf_document_tokens] || []

    same?(item.row_date, filter["row_date"] || filter[:row_date]) and
      (allowed == [] or item.pdf_document_token in allowed)
  end
  ...
  defp summarize_ko(item, pdf_text) do
    "#{item.issuer_name} filed a TDnet broad timely disclosure: #{item.title}. #{String.slice(normalize_space(pdf_text), 0, 260)}"
  end
