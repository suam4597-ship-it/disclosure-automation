defmodule DisclosureAutomation.Runtime.SECAdapter do
  ...
  def discover(%SourceRegistry{} = source, opts \\ []) do
    ...
    with {:ok, payload} <- load_discovery_payload(source, use_live_fetch) do
      {:ok, parse_atom_feed(payload, form_types)}
    end
  end
  ...
  def hydrate(%SourceRegistry{} = source, discovery_item, opts \\ []) do
    use_live_fetch = Keyword.get(opts, :use_live_fetch, true)
    fixtures = source.config["fixtures"] || source.config[:fixtures] || %{}
    detail_pages = fixtures["detail_pages"] || fixtures[:detail_pages] || %{}
    submission_texts = fixtures["submission_texts"] || fixtures[:submission_texts] || %{}

    detail_fixture_path =
      fixture_lookup(detail_pages, discovery_item.accession_no) ||
        fixture_detail_path_from_url(discovery_item.detail_url)

    submission_fixture_path =
      fixture_lookup(submission_texts, discovery_item.accession_no_nodash) ||
        fixture_submission_path_from_url(discovery_item.submission_text_url)

    with {:ok, detail_payload} <-
           load_document(discovery_item.detail_url, detail_fixture_path, use_live_fetch),
  ...
  def parse(%SourceRegistry{} = source, hydrated_item, _opts \\ []) do
    ...
    if form_type not in supported_forms(source) do
      ...
      accepted_at =
        hydrated_item.accepted_at_utc ||
          parse_submission_datetime(submission_text) ||
       ...
       [
         %{
           ...
           payload: %{
             ...
             "atom_updated_at" =>
               iso8601(discovery_item.atom_updated_at || discovery_item.accepted_at),
    ...
    contract_v1 = %{
      ...
      "portable_citations" => [
        %{
          ...
          "claim_supported" =>
            "accession, filing form, accepted time, and detail index anchor",
  ...
  defp supported_forms(source) do
    source.config["supported_forms_now"] || source.config[:supported_forms_now] || ["6-K"]
  end

  defp planned_forms(source) do
    source.config["planned_forms_next"] || source.config[:planned_forms_next] || ["8-K", "SC TO-T", "SC 14D-9", "SC 13D/A"]
  end

  defp load_discovery_payload(source, true) do
    discovery_fixture =
      get_in(source.config, ["fixtures", "discovery_feed"]) ||
        get_in(source.config, [:fixtures, :discovery_feed])
    ...
    with {:ok, response} <- Http.fetch(source.base_url, timeout: 8_000),
      ...
      _ -> load_fixture(source, discovery_fixture)
    end
  end
  ...
  defp load_discovery_payload(source, false) do
    ...
    load_fixture(source, discovery_fixture)
  end
  ...
  defp parse_atom_entry(entry) do
    href = xpath_string(entry, ~c"string(*[local-name()='link']/@href)") || ""
    ...
    accepted = accepted_times_or_fallback_from_detail(href, parse_iso8601(updated))
    ...
    %{
      ...
      accepted_at: accepted.accepted_at_utc,
      accepted_at_local: accepted.accepted_at_local,
      accepted_time_fallback: accepted.accepted_time_fallback,
      detail_url: href,
      submission_text_url:
        "https://www.sec.gov/Archives/edgar/data/#{cik}/#{accession_no_nodash}/#{accession_no_nodash}.txt",
      company_name: headline |> String.split(" - ") |> List.first(),
      cik: cik,
      form_type: form_type_from_title(headline),
      headline: headline
    }
  end
  ...
  defp summarize_ko("6-K", excerpt),
    do: "Form 6-K submission text summary: #{excerpt}"

  defp summarize_ko(form_type, excerpt),
    do: "#{form_type} submission text summary: #{excerpt}"

  defp why_important_ko("6-K"), do: "Foreign issuer material update."
  defp why_important_ko("8-K"), do: "Current report update."
  defp why_important_ko("SC TO-T"), do: "Tender offer filing."
  defp why_important_ko("SC 14D-9"), do: "Target response statement."
  defp why_important_ko("SC 13D/A"), do: "Control and ownership watch."
  defp why_important_ko(_), do: "Important SEC filing."

  defp headline_ko("6-K"), do: "Foreign issuer report (Form 6-K)"
  defp headline_ko("8-K"), do: "Current report (Form 8-K)"
  defp headline_ko("SC TO-T"), do: "Third-party tender offer statement (SC TO-T)"
  defp headline_ko("SC 14D-9"), do: "Solicitation or recommendation statement (SC 14D-9)"
  defp headline_ko("SC 13D/A"), do: "Beneficial ownership amendment (13D/A)"
  ...
  defp accepted_times_or_fallback_from_detail(detail_url, atom_updated_at) do
    case File.read(
           Path.expand(
             fixture_detail_path_from_url(detail_url),
  ...
  defp fixture_lookup(map, key) when is_map(map) do
    Map.get(map, key) || Map.get(map, to_string(key)) || lookup_integer_key(map, key)
  end

  defp fixture_lookup(_, _), do: nil

  defp lookup_integer_key(map, key) when is_binary(key) do
    case Integer.parse(key) do
      {int_key, ""} -> Map.get(map, int_key)
      _ -> nil
    end
  end

  defp lookup_integer_key(_map, _key), do: nil

  defp fixture_submission_path_from_url(url) do
    filename =
      url
      |> String.split("/")
      |> List.last()

    "source_payloads/sec_#{filename}"
  end
  ...
  defp fixture_detail_path_from_url(url) do
    ...
    "source_payloads/sec_#{accession}_index.html"
  end
