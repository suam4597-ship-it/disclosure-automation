-- SEC thin slice storage-level dedupe verification
-- Run after first poll and again after repeated poll with the same fixture/live payload.

-- 1) immutable raw document external accession dedupe
SELECT
  source_registry_id,
  external_id,
  COUNT(*) AS row_count
...
-- 2) detail/submission immutable document identity dedupe
SELECT
  source_registry_id,
  document_identity,
  document_type,
...
-- 7) current verified accession spot check for the 6-K fixture
SELECT
  external_id,
  COUNT(*) AS row_count
FROM raw_documents
WHERE external_id = '0001193125-26-135605'
GROUP BY external_id;

-- Expected: no rows from queries 1-6, and row_count = 1 from query 7.
