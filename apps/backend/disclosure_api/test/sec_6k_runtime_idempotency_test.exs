defmodule DisclosureAutomation.SEC6KRuntimeIdempotencyTest do
  ...
  test "6-K fixture path closes end-to-end and stays idempotent on repeated poll" do
    assert {:ok, first} =
    ...
    assert item1["published_at_local"] == "2026-04-01T06:15:02-04:00"
    assert String.starts_with?(item1["published_at_utc"], "2026-04-01T10:15:02")
    assert item1["filing_date_local"] == "2026-04-01"
             ...
             from(d in RawDocument,
               where:
                 d.source_registry_id == ^source.id and
                   d.external_id == ^"0001193125-26-135605:submission-text"
             ),
             :count
           ) == 1
    ...
    assert Repo.aggregate(
             from(d in RawDocument,
               where:
                 d.source_registry_id == ^source.id and
                   d.external_id == ^"0001193125-26-135605:detail-index"
             ),
             :count
           ) == 1
             ...
             from(e in RawEvent,
               where:
                 e.source_registry_id == ^source.id and
                   e.event_key == ^"sec:6-k:0001193125-26-135605"
             ),
             :count
           ) == 1
