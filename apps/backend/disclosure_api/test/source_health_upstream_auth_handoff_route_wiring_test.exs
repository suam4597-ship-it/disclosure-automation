defmodule DisclosureAutomation.SourceHealthUpstreamAuthHandoffRouteWiringTest do
  use DisclosureAutomationWeb.ConnCase, async: false

  alias DisclosureAutomation.Sources

  @source_key "source_health_upstream_auth_handoff_route_wiring_fixture"
  ...
  setup do
    ...
    {:ok, _source} =
      Sources.upsert_source(%{
        ...
        "display_name" => "Source Health Upstream Auth Handoff Route Wiring Fixture",
        "source_type" => "api",
        ...
        "base_url" => "https://example.test/source-health-upstream-auth-handoff-route-wiring",
        "healthcheck_url" => "https://example.test/source-health-upstream-auth-handoff-route-wiring/health",
        "parser_key" => "test_parser",
        "poll_cron" => "0 * * * *",
        "coverage_tags" => ["source_health", "auth", "upstream_handoff_route_wiring"],
        "active" => true,
        "config" => %{},
  ...
  test "upstream operator handoff enables bounded UI recheck action", %{conn: conn} do
    response =
      conn
      |> put_upstream_source_health_assigns(["source_health_operator"])
  ...
  test "upstream viewer handoff keeps UI recheck disabled despite query escalation", %{conn: conn} do
    response =
      conn
      |> put_upstream_source_health_assigns(["source_health_viewer"])
  ...
  test "upstream operator handoff authorizes backend recheck with fallback disabled", %{conn: conn} do
    response =
      conn
      |> put_upstream_source_health_assigns(["source_health_operator"])
  ...
  test "missing upstream handoff denies backend recheck despite body actor permissions", %{conn: conn} do
    response =
      conn
      |> post("/api/admin/source-health/#{@source_key}/recheck", recheck_payload())
      |> json_response(403)
  ...
  test "upstream recheck-only handoff does not authorize poll", %{conn: conn} do
    response =
      conn
      |> put_upstream_source_health_assigns(["source_health_operator"])
  ...
  test "upstream poll handoff passes poll authorization and reaches idempotency gate", %{conn: conn} do
    response =
      conn
      |> put_upstream_source_health_assigns(["source_health_poll_operator"])
  ...
  defp put_upstream_source_health_assigns(conn, role_names) do
    conn
    |> Plug.Conn.assign(:upstream_actor_id_hash, "sha256:upstream-route-actor-001")
    |> Plug.Conn.assign(:upstream_request_id_hash, "sha256:upstream-route-request-001")
    |> Plug.Conn.assign(:upstream_session_id_hash, "sha256:upstream-route-session-001")
    |> Plug.Conn.assign(:upstream_role_names, role_names)
  end
  ...
  defp recheck_payload do
    %{
      ...
      "idempotency_key_hash" => "sha256:upstream-route-recheck-idempotency-001",
  ...
  defp poll_payload do
    %{
      ...
      "idempotency_key_hash" => "sha256:upstream-route-poll-idempotency-001",
      "redaction_status" => "passed",
      "created_at" => "2026-05-06T00:00:00Z"
    }
  end
