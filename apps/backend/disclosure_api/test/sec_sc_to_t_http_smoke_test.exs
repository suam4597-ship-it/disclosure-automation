defmodule DisclosureAutomationWeb.SECSCToTHttpSmokeTest do
  ...
  test "health, poll, hero, region, digest, event, and source health close for SC TO-T fixture", %{conn: conn} do
    ...
    assert item["published_at_local"] == "2026-02-26T11:11:11-05:00"
    assert String.starts_with?(item["published_at_utc"], "2026-02-26T16:11:11")
    assert item["filing_date_local"] == "2026-02-26"
    assert get_in(item, ["source_meta", "accepted_time_fallback"]) == false
    assert item["event_family"] == "cash_tender_offer"
