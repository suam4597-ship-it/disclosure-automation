defmodule DisclosureAutomation.SEC8KRuntimeIdempotencyTest do
  ...
  test "8-K fixture path closes end-to-end and stays idempotent on repeated poll" do
    assert {:ok, first} =
    ...
    [item1] = digest1["items"]
    assert item1["published_at_local"] == "2026-02-25T15:20:28-05:00"
    assert String.starts_with?(item1["published_at_utc"], "2026-02-25T20:20:28")
    assert item1["filing_date_local"] == "2026-02-25"
    assert get_in(item1, ["source_meta", "accepted_time_fallback"]) == false
    assert item1["event_family"] == "current_report"
             ...
             from(d in RawDocument,
               where:
                 d.source_registry_id == ^source.id and
                   d.external_id == ^"0001140361-26-006772:submission-text"
             ),
             :count
           ) == 1
    ...
    assert Repo.aggregate(
             from(d in RawDocument,
               where:
                 d.source_registry_id == ^source.id and
                   d.external_id == ^"0001140361-26-006772:detail-index"
             ),
             :count
           ) == 1
             ...
             from(e in RawEvent,
               where:
                 e.source_registry_id == ^source.id and
                   e.event_key == ^"sec:8-k:0001140361-26-006772"
             ),
             :count
           ) == 1
    ...
    assert Enum.any?(cursors, &(&1.cursor_value == "0001140361-26-006772"))
  end
end
