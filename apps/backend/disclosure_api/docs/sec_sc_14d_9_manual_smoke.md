# SEC SC 14D-9 manual smoke

Use this checklist after the SC 14D-9 isolated runtime patch is in place.

## Base endpoint
- http://127.0.0.1:4000

## Prerequisite
Run the dedicated SC 14D-9 dev server runner so startup bootstrap also uses the isolated SC 14D-9 sample.

### PowerShell
1. `$env:POSTGRES_USER="postgres"`
2. `$env:POSTGRES_PASSWORD="<your-password>"`
3. `$env:MIX_ENV="dev"; mix.bat ecto.reset`
4. `mix.bat run --no-start priv/ops/run_sec_sc_14d_9_server.exs`
...
- published_at_local remains 2026-03-03T10:15:45-05:00
- published_at_utc remains 2026-03-03T15:15:45Z
- filing_date_local remains 2026-03-03
- accepted_time_fallback remains false on the verified detail path
- fact_summary_ko does not contain SEC closing tags
- record the exact `event_family` and canonical event type from the first green run before tightening assertions

## Required follow-up
After the repeated poll, run priv/ops/sec_sc_14d_9_dedupe_checks.sql.
Do not move to SC 13D/A or any regional/overlay work until the SQL checks are clean.
