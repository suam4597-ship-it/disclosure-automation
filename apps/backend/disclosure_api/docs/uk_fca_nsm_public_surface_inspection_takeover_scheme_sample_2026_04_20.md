# UK FCA NSM public surface inspection — takeover / scheme sample (2026-04-20)

This file records inspection findings using the user-supplied NSM CSV probe plus public RNS mirror confirmation.

## Candidate family under inspection

- [x] `takeover / scheme related update`
- [ ] `major holdings / director dealings`

## Search surface capture

- search entry URL: `public FCA NSM search UI`
- search filters used: `Document Text = scheme of arrangement`, `All words match`
- search query string: `scheme of arrangement`
- date range used: `2026-04-20 00:00` to `2026-04-23 08:29` on filing date basis
- whether the result can be exported to CSV: `yes (user supplied CSV export)`

## One deterministic result row

- row headline / title: `Scheme of Arrangement Becomes Effective`
- issuer / company name: `BRITISH LAND COMPANY PUBLIC LIMITED COMPANY(THE)`
- displayed date / time: `Filing 20/04/2026 06:13`, `Publication 20/04/2026 06:00`, `Document Date 20/04/2026`
- displayed announcement type / category: `Scheme of Arrangement`
- detail / artefact URL: `https://data.fca.org.uk/artefacts/NSM/RNS/5c9e4a51-b4c6-4977-86d3-ac8567261289.html`
- any explicit public id in result row: `artefact token visible in download link`
- any RNS number in result row: `not present in CSV row`
- any unique announcement id in result row: `not present in CSV row`

## Detail / artefact capture

- detail URL: `https://data.fca.org.uk/artefacts/NSM/RNS/5c9e4a51-b4c6-4977-86d3-ac8567261289.html`
- immutable token in URL: `5c9e4a51-b4c6-4977-86d3-ac8567261289`
- public id on detail page: `NSM artefact URL token`
- RNS number on mirrored RNS page: `0943B`
- unique announcement id on public page: `not observed in CSV or mirrored public RNS page`
- issuer name on mirrored RNS page: `British Land Co PLC`
- published timestamp on CSV row: `Publication 20/04/2026 06:00`; mirrored public RNS page observed with `20 April 2026 07:01:37` on one aggregator
- any linked filing payload URL: `not required for v0 based on public RNS body mirror; keep open for direct NSM artefact inspection`
- whether detail page alone is sufficient for normalization: `provisional yes, if NSM artefact page body matches mirrored RNS body`

## Identity decision

- chosen stable external identity field: `NSM namespace + source + artefact token`
- chosen stable external identity value: `NSM:RNS:5c9e4a51-b4c6-4977-86d3-ac8567261289`
- why this is better than the other candidates: `visible in public download link and independent of title text`
- whether this field is visible in both discovery and detail: `discovery = yes`, `detail = yes in URL`
- whether this field survives corrections / related update chains: `provisional; CSV exports latest rows, correction/version policy remains a later hardening item`

## Cursor decision

- chosen cursor key: `latest_filing_at_and_artefact_id_seen`
- chosen cursor source field: `Filing Date/Time + artefact token from Download Link`
- chosen cursor value: `2026-04-20T06:13:00|RNS|5c9e4a51-b4c6-4977-86d3-ac8567261289`
- why this is stable enough: `uses exported metadata rather than title text`
- why title text is not needed: `CSV already exposes a filing timestamp and deterministic artefact URL`

## Raw-document minimum set

- discovery result payload needed? `yes`
- detail artefact page needed? `yes`
- linked filing payload needed? `no for v0 unless the direct NSM artefact page proves thinner than the mirrored public RNS body`
- minimum raw-document count per item: `2`

## Family boundary check

- chosen event family: `takeover_or_scheme_update`
- adjacent family risk: `text query overmatches Result of Meeting and Offer Update rows`
- reason this item does not belong to a broader mixed bucket: `exact Category = Scheme of Arrangement`; body confirms the scheme became effective for British Land's recommended cash and share offer for Life Science REIT

## Freeze outcome

- [x] family passes contract-freeze exit criteria for an isolated v0 fixture implementation
- [ ] family still needs one direct detail-page inspection before contract freeze is complete

## Next action

- [x] open the isolated runtime implementation PR for this family
- [ ] inspect the direct NSM artefact page again during first-run fixture capture if fetch access allows
