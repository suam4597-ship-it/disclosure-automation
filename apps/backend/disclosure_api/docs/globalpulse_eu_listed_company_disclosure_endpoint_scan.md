...
## Candidate C5b: Denmark DFSA OAM Company Announcements API

```text
owner: Danish Financial Supervisory Authority
authority class: official appointed mechanism / company-announcement database
candidate page: https://www.dfsa.dk/financial-themes/capital-market/company-announcements
machine-readable URL: https://appft.gold.extension.gopublic.dk/api/9217fa13-5d9a-46c6-9921-69ee7e6cfaf6/search
observed HTTP: page 200; JSON API 200
observed content-type: API application/json
observed shape: JSON payload with paging and data.rows records containing id, HeadlineColumn, IssuerColumn, CategoryColumn, PublicationDateColumn, and RegistrationDateColumn
...
```text
DFSA describes the OAM database as containing publicly available announcements submitted to the Danish FSA, including company announcements from issuers.
The API returns issuer names, announcement headlines, publication timestamps, categories, and stable detail IDs for issuer/prospectus/regulated-information style announcements.
This is listed-company disclosure material, not a central-bank, macro, policy, or generic market commentary feed.
```

Implementation status:

```text
Parser dfsa_oam_company_announcements_json_v1 exists.
Manual source dk_dfsa_oam_company_announcements exists with active=false and candidate_status=manual_staging_only.
The source uses a bounded JSON POST request with page=1, pageSize=25, PublicationDateColumn descending sort, and a CategoryFilter allowlist that excludes ShortSelling.
Fixture source_payloads/dk_dfsa_oam_company_announcements.json captures the bounded public JSON shape.
Local parser smoke passed with fixture_records=5, live HTTP 200, live_records=10, and the first live row populated headline/issuer/category/url/published_at.
The extension API is sensitive to request body field order on the Erlang :httpc path, so the source stores live_body as an ordered JSON string matching the browser request shape.
Fly staging live poll passed with live HTTP 200, 25 records seen/inserted, and healthy source status.
Date-specific digest and latest public UI visibility remain pending because top-N/latest selection does not yet include Denmark DFSA OAM rows.
Cadence/rate/pagination design is recorded in globalpulse_denmark_dfsa_oam_cadence_rate_pagination_design.md.
Scheduled polling remains disabled until cadence, pagination, rate-limit, and repeated staging smoke evidence are documented.
```

## Candidate C5c: Ireland Euronext Dublin CAO / OAM Company Announcements

```text
owner: Euronext Dublin Company Announcements Office
authority class: official OAM / company-announcement dissemination surface
authority page: https://www.euronext.com/en/data/announcement-services-ireland
public company-news page: https://live.euronext.com/en/products/equities/company-news
global RSS URL: https://live.euronext.com/rss/company-pr-release
observed HTTP: company-news page 200; global RSS 200; Drupal AJAX probe 200
observed content-type: HTML page, RSS XML feed, Drupal AJAX JSON/HTML payload
scope fit: listed-company regulatory announcements for Dublin markets / Ireland OAM
machine-readable Dublin-only filter: not proven
status: AUTHORITY_SURFACE_CONFIRMED_SOURCE_REGISTRATION_BLOCKED
```
...
```text
Prove a Dublin-only RSS/JSON/XML/CSV endpoint, a documented Euronext query contract, or a bounded AJAX request whose count and rows change to Dublin-only.
Do not add Central Bank macro/policy feeds, third-party aggregators, or the unfiltered global Euronext RSS as an Ireland source.
```
...
observed HTTP: Fly staging support page 200 HTML; search-token 200; tokenized result page 200 HTML
observed shape: Next.js/React embedded HTML with elasticSearch, publicationDto, sourceDate, and companyNameAtTimeOfPublication markers after search-token preflight
current marker caveat: exact searchResults.elasticSearchDtos marker was not present in the Fly staging tokenized HTML, so the candidate parser extracts the current escaped searchResults payload shape instead
status: MANUAL_SOURCE_REGISTERED_STAGING_LIVE_POLL_PASS_DATE_SPECIFIC_DIGEST_VISIBILITY_PASS
date/detail update: ISO sourceDateFrom/sourceDateTo query and /en/publication?payload=<encryptedPayload> detail route confirmed from Fly staging
```
...
The tokenized result page exposes publicationDto-style rows with company name, source date, and navigation payload fields.
The ISO date-range query uses sourceDateFrom=YYYY-MM-DD and sourceDateTo=YYYY-MM-DD.
The public canonical detail route is /en/publication?payload=<encryptedPayload> and does not retain searchToken.
...
Daily date windows can exceed one page; a 2024-09-30 probe returned Page 1 of 7 with 188 total results.
```

Manual candidate implementation:

```text
The token-preflight contract is recorded in globalpulse_germany_company_register_token_preflight_contract.md.
The staging-network preflight is recorded in globalpulse_germany_company_register_staging_network_preflight_results.md.
The pagination/rate/captcha design is recorded in globalpulse_germany_company_register_pagination_rate_captcha_design.md.
Manual source de_company_register_capital_market_info exists with active=false and candidate_status=manual_staging_only.
Parser germany_company_register_capital_market_flight_v1 consumes the source-specific token preflight fan-out JSON envelope.
Fixture source_payloads/de_company_register_capital_market_info.json records 2024-09-30 rows from the proven daily ISO query.
Local parser smoke parsed 3 fixture records; the first record has title, generated publication URL, UTC date-only published_at, category, and stable external_id.
The live fetch adapter performs support page -> search-token -> tokenized ISO date query and records total_pages, total_results, pages_fetched, records_seen, and over_page_cap.
The manual staging smoke window is intentionally capped at max_pages_per_poll=1 and max_items_per_poll=25; the 2024-09-30 evidence remains over-cap with totalPages=7 and totalResults=188.
Staging live poll smoke passed with fetch.mode=live, support/token/search HTTP 200, fixture fallback=false, fetch.records_seen=30, records_inserted=25, canonical_items=25, totalPages=7, totalResults=188, and over_page_cap=true.
Date-specific digest GET /api/feed/digest/2024-09-30/breaking rendered 12 Germany Company Register items under eu_central with fetch_mode=live.
...
Rate/captcha and over-page behavior are designed, but any multi-page staging smoke and scheduled-poll promotion remain pending.
...
status: ISSUER_NEWS_MANUAL_SOURCE_REGISTERED_STAGING_LIVE_POLL_PASS_REPORT_CALENDAR_MANUAL_SOURCE_REGISTERED_STAGING_LIVE_POLL_PASS_SCHEDULED_POLLING_DISABLED
```
...
Report-calendar Fly staging deploy passed with image registry.fly.io/globalpulse-backend-staging:deployment-01KR6GGP6M143CQ09SSVY7C6JA and release_command success.
Report-calendar staging live poll passed with fetch.mode=live, fetch.strategy=pse_multi_isin_report_calendar_v1, fetch.status_code=200, universe_count=63, selected_issuer_count=10, calendar_request_count=10, records_seen=20, records_inserted=20, canonical_items=20, and fixture fallback=false.
Report-calendar date-specific digest visibility passed: GET /api/feed/digest/2026-04-30/breaking, GET /api/feed/digest/2026-04-28/breaking, and GET /api/feed/digest/2026-04-27/breaking rendered PSE issuer report rows under eu_central with fetch_mode=live.
Request budget, source separation, and issuer-window rotation are designed, but repeated rotation smoke evidence and any scheduled-poll promotion remain pending.
...
Fly staging deploy and live poll smoke passed with fetch.mode=live, fetch.status_code=200, records_seen=9, and records_inserted=9.
Date-specific digest visibility passed for Malta rows in 2026-05-07, 2026-05-06, and 2026-05-05 digests.
Scheduled polling remains disabled until the broader EU source batch is intentionally promoted.
```

## Candidate Z: Bulgaria X3News Issuer Disclosures

```text
owner: Financial Market Services / X3News
authority class: Bulgarian Stock Exchange group issuer-disclosure media surface
supporting URL: https://fms.capital/en/aboutus
candidate URL: https://www.x3news.com/?language=en
observed HTTP: 200
observed content-type: text/html;charset=UTF-8
observed shape: server-rendered issuer-disclosure rows with issuer, disclosure headline, date/time, and ExtriID detail identifiers
status: MANUAL_SOURCE_REGISTERED_STAGING_LIVE_POLL_PASS_DATE_SPECIFIC_DIGEST_VISIBILITY_PASS_PUBLIC_LATEST_UI_VISIBILITY_PENDING_SCHEDULED_POLLING_DISABLED
```

Why this fits the product:

```text
FMS states that it is part of the Bulgarian Stock Exchange group.
FMS states that X3News is a specialized media portal through which issuers of financial instruments fulfill their legal obligations to disclose regulated information to the public.
The English X3News page exposes listed issuer names, disclosure categories such as inside information and quarterly reports, publication timestamps, and official ShowNews detail identifiers.
This is a listed-company issuer-disclosure source, not a central-bank, macro, parliament, or policy-news feed.
```
...
Fly staging deploy and live poll smoke passed with fetch.mode=live, fetch.status_code=200, records_seen=11, records_inserted=11, and fixture fallback disabled.
Date-specific digest visibility passed for Bulgaria rows in 2026-05-07, 2026-05-06, and 2026-05-05 digests.
Public latest UI visibility remains pending because the public shell currently renders the newer 2026-05-09 latest digest window.
Scheduled polling remains disabled until the broader EU source batch is intentionally promoted.
```
...
status: MANUAL_SOURCE_REGISTERED_LOCAL_AND_LIVE_PARSER_SMOKE_PASS_STAGING_LIVE_POLL_PASS_LATEST_BACKEND_DIGEST_VISIBILITY_PASS_PUBLIC_BROWSER_VISUAL_SMOKE_PENDING_SCHEDULED_POLLING_DISABLED
```
...
Fly staging live poll smoke passed on 2026-05-10 against phase0-foundation commit 95e09601f7850b2ccb62f2faad87a0262761e3fe.
The staging poll returned 202, fetch.mode=live, status_code=200, 6,018,178 bytes, records_seen=25, and records_inserted=25.
Latest backend digest visibility passed for digest_date 2026-05-09 with Turkey KAP items in the top-N response.
Turkey canonical region label smoke passed after PR #467: KAP items now render with `regions=tr`, and Pages config maps `tr` to `Turkey`.
Scheduled polling remains disabled until the broader Europe source batch is intentionally promoted.
```

## Candidate AB: Cyprus CSE OAM Public Listing Versions

```text
owner: Cyprus Stock Exchange / XAK Public OAM
authority class: official OAM / listed-company regulated-information surface
...
observed HTTP: 200
observed content-type: application/json; charset=UTF-8
observed shape: paged JSON content with listedCompanyEnglish, nameEnglish, infoCategoriesNameEnglish, securityCodesCode, publicationTimestamp, translationVersionContent/versionContent, and public card detail identifiers
status: MANUAL_SOURCE_REGISTERED_LOCAL_AND_LIVE_PARSER_SMOKE_PASS_STAGING_LIVE_POLL_PASS_DATE_SPECIFIC_DIGEST_VISIBILITY_PASS_LATEST_PUBLIC_UI_VISIBILITY_PENDING_SCHEDULED_POLLING_DISABLED
```

Why this fits the product:

```text
The CSE announced an OAM RSS service for announcements/news published on the CSE OAM system and stated that the flows contain all published announcements of listed CSE companies among other market participants.
The current public OAM SPA exposes an official JSON endpoint used by the public listing-version screen.
The JSON payload contains listed-company fields, security codes, category names, timestamps, and bounded announcement content suitable for canonical public digest items.
This is a listed-company regulated-information source, not a central-bank, macro, parliament, or policy-news feed.
```

Implementation status:

```text
Parser cse_oam_listing_versions_json_v1 exists.
Manual source eu_cyprus_cse_oam exists with active=false and candidate_status=manual_staging_only.
Fixture source_payloads/eu_cyprus_cse_oam.json captures the bounded public listing-version API shape.
The source has disable_live_fixture_fallback=true so staging must prove live fetch success before any live-poll claim.
Local fixture parser smoke passed with 3 bounded records.
Live parser smoke passed against the candidate URL with HTTP 200 application/json, 91,458 bytes, and 21 listed-company records kept from the bounded 25-row page.
Fly staging live poll smoke passed on 2026-05-10 against phase0-foundation commit 369c8247d8e6c00b0df5f4324b065fd85501b652.
The staging poll returned 202, fetch.mode=live, status_code=200, 91,458 bytes, records_seen=21, and records_inserted=21.
Date-specific digest visibility passed for Cyprus rows in 2025-04-02, 2025-03-31, 2024-02-05, and 2023-04-13 digests under eu_south.
Latest public UI visibility remains pending because the public latest digest currently renders 2026-05-09 while the bounded CSE OAM page latest Cyprus row is 2026-04-30.
Scheduled polling remains disabled until the broader Europe source batch is intentionally promoted.
```

## Candidate AC: Banja Luka Stock Exchange Issuer News RSS

```text
owner: Banja Luka Stock Exchange
authority class: official exchange issuer-news/export surface
supporting URL: https://www3.blberza.com/Pages/docview.aspx?page=sp99
ticker URL: https://services.blberza.com/blse/ticker.ashx?LangId=3&TickerTypeId=1&filter=all&ct=xml
issuer RSS URL template: https://www.blberza.com/pages/IssuerNewsRss.aspx?Code={code}&LangId=3
observed ticker HTTP: 200
observed ticker content-type: application/xml; charset=utf-8
observed issuer RSS HTTP: 200
observed issuer RSS content-type: application/rss+xml; charset=utf-8
observed shape: ticker XML exposes TickerItem Code, Issuer, and Url; per-issuer RSS exposes title, link, pubDate, and description
status: MANUAL_SOURCE_REGISTERED_LOCAL_PARSER_SMOKE_PASS_LIVE_ENDPOINT_PROBE_PASS_STAGING_LIVE_POLL_PASS_DATE_SPECIFIC_DIGEST_VISIBILITY_PASS_LATEST_PUBLIC_UI_VISIBILITY_PENDING_SCHEDULED_POLLING_DISABLED
```

Why this fits the product:

```text
The BLSE public documentation describes issuer announcement export endpoints and an issuer RSS endpoint keyed by security code.
The ticker XML provides bounded official security-code discovery; the implementation filters to listed equity codes ending in -R-A.
The issuer RSS records contain listed-company announcement titles, official DocView links, publication dates, and descriptions.
This is a listed-company exchange announcement source, not a central-bank, macro, parliament, or broad policy-news feed.
```

Implementation status:

```text
Parser blse_multi_issuer_news_rss_v1 exists.
Manual source ba_blse_issuer_news_multi_code exists with active=false and candidate_status=manual_staging_only.
Fixture source_payloads/ba_blse_issuer_news_multi_code.json captures the bounded fan-out JSON shape.
The source has disable_live_fixture_fallback=true so staging must prove live fetch success before any live-poll claim.
Local fixture parser smoke passed with 4 bounded records.
Live endpoint probe passed for BLSE ticker XML and TLKM-R-A issuer RSS.
Fly staging live poll smoke passed on 2026-05-10 against phase0-foundation commit 3032c738e01315bb61ee1c075a31fbb05ca2f739.
The staging poll returned 202, fetch.mode=live, ticker_status_code=200, status_code=200, 67,200 bytes, universe_count=14, selected_issuer_count=5, issuer_request_count=5, fetch.records_seen=75, records_seen=25, and records_inserted=25.
Date-specific digest visibility passed for BLSE rows in 2026-05-04, 2026-04-30, 2026-04-28, and 2026-04-22 digests under eu_south.
Latest public UI visibility remains pending because the public latest digest currently renders 2026-05-09 while the bounded BLSE issuer window latest row is 2026-05-08.
Scheduled polling remains disabled until the broader Europe source batch is intentionally promoted.
```

## Candidate AD: Macedonian Stock Exchange Free Market Announcements
...
observed HTTP: 200
observed content-type: text/html; charset=utf-8
observed shape: Issuers page contains a bounded "Announcements from companies on the Free Market" HTML list with announcement URL, date, issuer/title, and announcement type
status: MANUAL_SOURCE_REGISTERED_LOCAL_AND_LIVE_PARSER_SMOKE_PASS_STAGING_LIVE_POLL_PASS_DATE_SPECIFIC_DIGEST_VISIBILITY_PASS_LATEST_PUBLIC_UI_VISIBILITY_PENDING_SCHEDULED_POLLING_DISABLED
```
...
```text
The candidate is an official Macedonian Stock Exchange issuer page, not a central-bank, macro, parliament, or policy-news feed.
The bounded row shape exposes listed-company/free-market issuer announcement material with official MSE announcement URLs.
The first slice intentionally uses only the latest public Free Market announcement list; detail/attachment fetching is out of scope.
```

Implementation status:

```text
Parser mse_free_market_announcements_html_v1 exists.
Manual source mk_mse_free_market_announcements exists with active=false and candidate_status=manual_staging_only.
Fixture source_payloads/mk_mse_free_market_announcements.html captures the bounded public row shape.
The source has disable_live_fixture_fallback=true so staging must prove live fetch success before any live-poll claim.
Live endpoint probe passed against https://www.mse.mk/Issuers.aspx with HTTP 200 text/html and the expected bounded row markers.
Local fixture parser smoke passed with 3 bounded records.
Live parser smoke passed with HTTP 200 text/html, 134,203 bytes, and 25 records after the parser capability limit.
Fly staging live poll smoke passed on 2026-05-10 against phase0-foundation commit 009aa66a5cb366a47905f87d71b348f1e1822133.
The staging poll returned 202, fetch.mode=live, status_code=200, 134,203 bytes, records_seen=25, and records_inserted=25.
Date-specific digest visibility passed for MSE rows in 2026-05-06 and 2026-05-05 digests under eu_south.
Latest public UI visibility remains pending because the public latest digest currently renders 2026-05-09 while the latest MSE row is 2026-05-08.
Scheduled polling remains disabled until the broader Europe source batch is intentionally promoted.
```

## Candidate AE: North Macedonia SEI-NET Public Documents

```text
owner: SEI-NET / Macedonian listed-company disclosure system
authority class: official public disclosure platform API
candidate URL: https://api.seinet.com.mk/public/documents
observed HTTP: 200
observed content-type: application/json
observed shape: POST body with channelId=1, languageId=2, page=1 returns isSuccess=true, totalSize, and bounded document rows containing documentId, publicId, issuer, layout/category, publishedDate, requestPublishDate, content, and attachments
...
```text
The candidate is an official listed-company disclosure API rather than a central-bank, macro, policy, or generic news feed.
The public document rows expose issuer announcements, regulated information, financial statements, assembly notices, and other listed-company disclosure material.
The parser intentionally ignores user/action-log/remote-address material present in the raw API response and emits only bounded disclosure fields.
```

Implementation status:

```text
Parser seinet_public_documents_json_v1 exists.
Manual source mk_seinet_public_documents exists with active=false and candidate_status=manual_staging_only.
Fixture source_payloads/mk_seinet_public_documents.json captures the bounded public documents response shape.
The source has disable_live_fixture_fallback=true so staging must prove live fetch success before any live-poll claim.
Local registry/capability smoke passed.
Local fixture parser smoke passed with 3 bounded records.
Live parser smoke passed with HTTP 200 application/json, 50,183 bytes, and 10 bounded records.
Fly staging live poll smoke passed on 2026-05-10 against phase0-foundation commit dc51edf0ea9505675c342fa1ea067f46f1c7ec89.
The staging poll returned 202, fetch.mode=live, status_code=200, 50,183 bytes, records_seen=10, records_inserted=10, and canonical_items_count=10.
Source health updated to healthy with last_seen_published_at=2026-05-08T15:47:00.177000Z.
Date-specific digest top-N visibility remains pending because the default 2026-05-08 digest did not include SEI-NET rows.
Scheduled polling remains disabled until the broader Europe source batch is intentionally promoted.
```

## Candidate AF: Montenegro Stock Exchange Corporate News
...
```text
The candidate is an official Montenegro Stock Exchange listed-company corporate announcement surface.
Rows expose issuer announcements such as shareholder assembly notices and other corporate announcements.
The first slice intentionally parses the latest public corporate-news list only; PDF/detail fetching is out of scope.
```

Implementation status:

```text
Parser mnse_corporate_news_html_v1 exists.
Manual source me_mnse_corporate_news exists with active=false and candidate_status=manual_staging_only.
Fixture source_payloads/me_mnse_corporate_news.html captures the bounded corporate-news row shape.
The source has disable_live_fixture_fallback=true so staging must prove live fetch success before any live-poll claim.
Local registry/capability smoke passed.
Local fixture parser smoke passed with 3 bounded records.
...
An HTTP fetch retry was tested, but the official page redirects to HTTPS with `301 Location: https://www.mnse.me/code/navigate.asp?Id=852`, so it does not avoid the blocked TLS path.
Scheduled polling remains disabled until the broader Europe source batch is intentionally promoted.
```
...
26. Keep Turkey KAP Company Notifications as a proven manual_staging_only official public-disclosure-platform candidate with local fixture/live parser smoke passing, Fly staging live poll passing, latest backend digest visibility passing, and Turkey region label smoke passing.
27. Keep Cyprus CSE OAM Public Listing Versions as a manual_staging_only official OAM JSON candidate with local/live parser smoke passing, Fly staging live poll passing, date-specific digest visibility passing, and latest public UI visibility pending.
...
PRAGUE_PSE_ISSUER_REPORT_CALENDAR_MULTI_ISIN_MANUAL_SOURCE_REGISTERED_STAGING_LIVE_POLL_PASS
PRAGUE_PSE_ISSUER_REPORT_CALENDAR_MULTI_ISIN_DATE_SPECIFIC_DIGEST_VISIBILITY_PASS
PRAGUE_PSE_CADENCE_RATE_DESIGN_RECORDED
GERMANY_COMPANY_REGISTER_CAPITAL_MARKET_TOKENIZED_SEARCH_CONFIRMED
GERMANY_COMPANY_REGISTER_MANUAL_SOURCE_REGISTERED_STAGING_LIVE_POLL_PASS
GERMANY_COMPANY_REGISTER_LOCAL_PARSER_SMOKE_PASS
...
TURKEY_KAP_COMPANY_NOTIFICATIONS_STAGING_LIVE_POLL_PASS
TURKEY_KAP_COMPANY_NOTIFICATIONS_LATEST_BACKEND_DIGEST_VISIBILITY_PASS
TURKEY_KAP_COMPANY_NOTIFICATIONS_CANONICAL_REGION_TR_PASS
TURKEY_KAP_COMPANY_NOTIFICATIONS_PAGES_TURKEY_LABEL_CONFIG_PASS
CYPRUS_CSE_OAM_PUBLIC_LISTING_VERSIONS_MANUAL_SOURCE_REGISTERED
CYPRUS_CSE_OAM_PUBLIC_LISTING_VERSIONS_LOCAL_PARSER_SMOKE_PASS
CYPRUS_CSE_OAM_PUBLIC_LISTING_VERSIONS_LIVE_PARSER_SMOKE_PASS
CYPRUS_CSE_OAM_PUBLIC_LISTING_VERSIONS_STAGING_LIVE_POLL_PASS
CYPRUS_CSE_OAM_PUBLIC_LISTING_VERSIONS_DATE_SPECIFIC_DIGEST_VISIBILITY_PASS
CYPRUS_CSE_OAM_PUBLIC_LISTING_VERSIONS_LATEST_PUBLIC_UI_VISIBILITY_PENDING
BANJA_LUKA_BLSE_ISSUER_NEWS_MULTI_CODE_MANUAL_SOURCE_REGISTERED
BANJA_LUKA_BLSE_ISSUER_NEWS_MULTI_CODE_LOCAL_PARSER_SMOKE_PASS
BANJA_LUKA_BLSE_ISSUER_NEWS_MULTI_CODE_LIVE_ENDPOINT_PROBE_PASS
BANJA_LUKA_BLSE_ISSUER_NEWS_MULTI_CODE_STAGING_LIVE_POLL_PASS
BANJA_LUKA_BLSE_ISSUER_NEWS_MULTI_CODE_DATE_SPECIFIC_DIGEST_VISIBILITY_PASS
BANJA_LUKA_BLSE_ISSUER_NEWS_MULTI_CODE_LATEST_PUBLIC_UI_VISIBILITY_PENDING
MACEDONIAN_MSE_FREE_MARKET_ANNOUNCEMENTS_MANUAL_SOURCE_REGISTERED
MACEDONIAN_MSE_FREE_MARKET_ANNOUNCEMENTS_LOCAL_PARSER_SMOKE_PASS
MACEDONIAN_MSE_FREE_MARKET_ANNOUNCEMENTS_LIVE_PARSER_SMOKE_PASS
MACEDONIAN_MSE_FREE_MARKET_ANNOUNCEMENTS_LIVE_ENDPOINT_PROBE_PASS
MACEDONIAN_MSE_FREE_MARKET_ANNOUNCEMENTS_STAGING_LIVE_POLL_PASS
MACEDONIAN_MSE_FREE_MARKET_ANNOUNCEMENTS_DATE_SPECIFIC_DIGEST_VISIBILITY_PASS
MACEDONIAN_MSE_FREE_MARKET_ANNOUNCEMENTS_LATEST_PUBLIC_UI_VISIBILITY_PENDING
MACEDONIAN_SEINET_PUBLIC_DOCUMENTS_MANUAL_SOURCE_REGISTERED
MACEDONIAN_SEINET_PUBLIC_DOCUMENTS_LOCAL_PARSER_SMOKE_PASS
MACEDONIAN_SEINET_PUBLIC_DOCUMENTS_LIVE_PARSER_SMOKE_PASS
MACEDONIAN_SEINET_PUBLIC_DOCUMENTS_STAGING_LIVE_POLL_PASS
MACEDONIAN_SEINET_PUBLIC_DOCUMENTS_DATE_SPECIFIC_DIGEST_TOP_N_VISIBILITY_PENDING
MONTENEGRO_MNSE_CORPORATE_NEWS_MANUAL_SOURCE_REGISTERED
...
MONTENEGRO_MNSE_CORPORATE_NEWS_HTTP_RETRY_REDIRECTS_TO_BLOCKED_TLS
...
DENMARK_DFSA_OAM_COMPANY_ANNOUNCEMENTS_MANUAL_SOURCE_REGISTERED
DENMARK_DFSA_OAM_COMPANY_ANNOUNCEMENTS_EXTERNAL_ENDPOINT_PROBE_PASS
DENMARK_DFSA_OAM_COMPANY_ANNOUNCEMENTS_LOCAL_PARSER_SMOKE_PASS
DENMARK_DFSA_OAM_COMPANY_ANNOUNCEMENTS_LIVE_PARSER_SMOKE_PASS
...
EU_CANARY_ARTIFACT_PAYLOAD_REVIEW_PENDING_AUTHENTICATED_DOWNLOAD
...
EU_NEXT_IMPLEMENTATION_STEP_EU_FIRST_AUTOMATED_CANARY_SMOKE
