# SEC SC TO-T manual smoke

Use this checklist after the SC TO-T isolated runtime patch is in place.

## Base endpoint
- http://127.0.0.1:4000

## Prerequisite
Run the dedicated SC TO-T dev server runner so startup bootstrap also uses the isolated SC TO-T sample.

### PowerShell
1. `$env:POSTGRES_USER="postgres"`
2. `$env:POSTGRES_PASSWORD="<your-password>"`
3. `$env:MIX_ENV="dev"; mix ecto.reset`
4. `mix run --no-start priv/ops/run_sec_sc_to_t_server.exs`
...
- published_at_local remains 2026-02-26T11:11:11-05:00
- published_at_utc remains 2026-02-26T16:11:11Z
- filing_date_local remains 2026-02-26
- event_family remains cash_tender_offer
- accepted_time_fallback remains false on the verified detail path
- fact_summary_ko does not contain SEC closing tags

## Required follow-up
After the repeated poll, run priv/ops/sec_sc_to_t_dedupe_checks.sql.
No further form or regional expansion should start until the SQL checks are clean.
